...
let downHeight = downChild.clientHeight,
...
oLine.onmousedown = function (ev) {
  ...
  let iEvent = ev || event;
  let dy = iEvent.clientY; //当你第一次单击的时候，存储y轴的坐标。//相对于浏览器窗口
  upperHeight = upperChild.offsetHeight;
  downHeight = downChild.offsetHeight;
  document.onmousemove = function (ev) {
    let iEvent = ev || event;
    let diff = iEvent.clientY - dy; //移动的距离（向上滑时为负数,下滑时为正数）
